...
# GitHub Copilot using grok-code-fast-1 Instructions for VS Code 
...
### Technical Patterns
```javascript
// Preferred patterns for consistency
...
const processData = async (data) => {
  try {
    ...
    throw new ProcessingError(`Failed processing: ${error.message}`, { 
      originalError: error,
...
### Rapid Verification
```markdown
**Test**: `npm test specific-test.js`
...
Build: [e.g., Webpack, Vite]
Style: [e.g., ESLint config, Prettier]
Constraints: [e.g., no external APIs, specific browser support]
